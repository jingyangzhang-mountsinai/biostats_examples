...
2. `BMT` Data Example
   ...
   c. Analysis:
      ...
      ii. Step 2: subset population for those followed at least until landmark time.
...
1. **Time-dependent covariate approach** is an alternative to a landmark analysis, which incorporates time-dependent covariates. 
  a. Time-dependent covariate approach is more appropriate than landmark analysis when:
    - The value of covariate is changing over time.
    - There is not an obvious landmark time. 
    - Use of a landmark would lead too many exclusions. 
...
![](images/bmt_countingprocessformat5.png)
      - We can see that patients 1 and 4 now have multiple rows of data because they both developed aGVHD at some point in time after baseline. 
    - Analyze this time-dependent covariate as usual using Cox regression with `coxph` and an alteration to our use of `Surv` to include arguments to both `time` and `time2`:
...
coxph(
  ...
  tbl_regression(exp = TRUE)
```
      - The result indicates that acute graft vs. host disease is not significantly associated with death using either landmark analysis or a time-dependent covariates. 


## Summary

1. Often one will want to use landmark analysis for visualization of a single covariate. 

2. One will use Cox regression with a time-dependent covariate for univariate and multivariate modeling. 
