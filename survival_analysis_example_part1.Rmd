---
title: "Survival Analysis in R - Part 1"
...
## time: time points where events (deaths) occurred.
## n.risk: number of individuals still at risk just before this time point. 
## n.event: number of events at this time point. 
## surv: Kaplan-Meier estimate of survival probability at this time. 
## std.err: standard error of the survival estimate. 
## lower/upper: confidence interval for survival probability. 
## strata: group (treatment) if you fitted multiple groups. 
```
...
```{r}
est_surv_1yr <- summary(survfit(Surv(time, status) ~ 1, data = lung), times = 365.25)
...
survfit2(Surv(time, status) ~ 1, data = lung) %>% 
  ...
  geom_hline(yintercept = est_surv_1yr$surv, color = "red", linetype = "dashed")
```
